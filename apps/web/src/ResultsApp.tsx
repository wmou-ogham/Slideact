...
export function ResultsApp({ t }: { t: Translate }) {
  ...
  useEffect(() => {
    ...
    return () => document.body.classList.remove("projection-body");
  }, [sessionId]);
  ...
  return (
    <main className="projection-root session-results-root">
      <header>
      ...
      {cueRuns.map((run) => {
        ...
        return (
          <section className={multi ? "projection-results projection-multi" : "projection-results"} key={run.cue_id}>
            ...
            {!multi && <h1>{run.interactions[0]?.prompt ?? run.cue_name}</h1>}
