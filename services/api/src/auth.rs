...
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct RestoreGuestVaultRequest {
    recovery_key: String,
}

#[derive(Debug, Serialize)]
struct GuestVaultFile {
    kind: &'static str,
    version: u8,
    vault_id: Uuid,
    recovery_key: String,
...
pub(crate) fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/auth/guest/export", post(export_guest_vault))
        .route("/api/auth/guest/restore", post(restore_guest_vault))
...
mod tests {
    ...
    use super::{
        GUEST_VAULT_RECOVERY_PREFIX, SESSION_COOKIE, build_cookie, hash_secret,
        is_plausible_recovery_key, random_token, read_cookie, sanitize_return_to, secrets_equal,
    };
